...
/**
 * GreyscaleHandler - Dual Mode Support
 *
 * This Lambda function converts images to greyscale and supports TWO invocation modes:
 *
...
async function handleS3Event(event, inspector) {
  ...
  try {
    ...
    // ----------------------------------------------------------
    // 3. WRITE PROCESSED IMAGE TO OUTPUT PREFIX
    // ----------------------------------------------------------
 ...
 *
 *     "operation": "greyscale"               // Operation name (for logging)
 * }
 *
 * Returns:
 * {
 *     "image_data": "base64_encoded_greyscale_image",
 *     "operation": "greyscale",
 *     "success": true,
...
async function handleDirectInvocation(event, inspector) {
  try {
    ...
    // Add operation info
    const operation = event.operation || "greyscale";
    ...
    // ----------------------------------------------------------
    // 3. ENCODE GREYSCALE IMAGE BACK INTO BASE64
    // ----------------------------------------------------------

    const resultB64 = greyscaleImageBuffer.toString('base64');

    // Add result info
    inspector.addAttribute("output_size", greyscaleImageBuffer.length);
