...
2. Create a new test event:
   ...
   ```
   Note: The function auto-detects images in the `input/` folder
3. Click **"Save"**
4. Click **"Test"**
...
Before testing, upload a test image to the `input/` folder:

```bash
aws s3 cp input_image.jpeg s3://tcss462-term-project-group-7/input/
```

Or upload through S3 Console:
1. Go to S3 Console
2. Open bucket `tcss462-term-project-group-7`
3. Create folder `input/` if it doesn't exist
4. Click **"Upload"**
5. Select your image file (`.jpg`, `.jpeg`, or `.png`)
6. Click **"Upload"**
...
1. **Rotate**: Processes `input/{filename}` → creates `stage1/{filename}`
2. **Resize**: Processes `stage1/{filename}` → creates `stage2/{filename}`
3. **Greyscale**: Processes `stage2/{filename}` → creates `output/{filename}`

The pipeline automatically detects and processes image files (`.jpg`, `.jpeg`, `.png`) in each folder.
